...
abstract class Field extends Widget
{
    ...
    public $insert_value;
    public $update_value;
    public $insert_description;
    public $update_description;
    public $show_value; //default value in visualization
    public $edited = false;
    public $options = [];
    public $mask;
    public $group;
    ...
    public function autoUpdate($save = false)
    {
        ...
        if (\is_object($this->model) && isset($this->db_name)) {
            if (
                ...
                $self = $this;
                $this->model->saved(static function () use ($self) {
    ...
    public function getValue()
    {
        ...
        if (true == $this->request_refill && $process && Request::exists($this->name)) {
            if ($this->multiple) {
                $this->value = '';
...
//                     @$this->model->$relation->$name;
                ...
                default:
                    ...
                    //polimorphic, etc..
                    throw new InvalidArgumentException("The field {$this->db_name} is a " . $methodClass . ' but Rapyd can handle only BelongsToMany, BelongsTo, and HasOne');
                    break;
            }
        } elseif ((isset($this->model)) && (null === Request::input($this->name)) && ($this->model->offsetExists($this->db_name))) {
    ...
    public function getMode()
    {
        switch ($this->mode) {
            case 'showonly':
                ...
                break;
            case 'autohide':
                if (('modify' == $this->status) || ('update' == $this->action)) {
                    $this->status = 'show';
            ...
            case 'autoshow':
                if (('create' == $this->status) || ('insert' == $this->action)) {
                    $this->status = 'hidden';
                    $this->apply_rules = false;
                }
                break;
        ...
        if (isset($this->when)) {
            if (\is_string($this->when) and mb_strpos($this->when, '|')) {
    ...
    public function getNewValue()
    {
        ...
        if ($process && Request::exists($this->name)) {
            if ('create' == $this->status) {
                $this->action = 'insert';
            } elseif ('modify' == $this->status) {
                $this->action = 'update';
            }
...
//        $this->star = (!($this->status == "show") and $this->required) ? '&nbsp;*' : '';
        $this->req = (!('show' == $this->status) and $this->required) ? ' required' : '';
        if (('hidden' == $this->status || false === $this->visible || \in_array($this->type, ['hidden', 'auto']))) {
            $this->is_hidden = true;
        }
        ...
        foreach ($attributes as $attribute) {
            ...
            if ('inline' == $this->orientation || $this->has_placeholder) {
                if ('select' != $this->type) {
    ...
    {
        $output = '';
        if ($this->has_wrapper && $this->has_label && 'inline' != $this->orientation) {
    ...
    protected function setName($name)
    {
        ...
        if (isset($this->model) &&
            ...
            is_a(@$this->model->$relation(), Relation::class)
        ) {
            ...
            if (is_a(@$this->relation, BelongsTo::class)) {
                $this->db_name = $this->relation->getForeignKeyName();
            } else {
                $this->db_name = $name;
            }

            if (is_a(@$this->relation, BelongsToMany::class)) {
    ...
    {
        if (\is_object($this->model) && (false !== mb_strpos($string, '{{') || false !== mb_strpos($string, '{!!') || $is_view)) {
