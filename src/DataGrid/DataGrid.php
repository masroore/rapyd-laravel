...
class DataGrid extends DataSet
{
    ...
    public function buildCSV($file = '', $timestamp = '', $sanitize = true, $del = [], $directToFile = false)
    {
        ...
        $save = (false === mb_strpos($file, '/')) ? false : true;

        //Delimiter
        $delimiter = [];
        $delimiter['delimiter'] = $del['delimiter'] ?? ';';
        $delimiter['enclosure'] = $del['enclosure'] ?? '"';
        $delimiter['line_ending'] = $del['line_ending'] ?? "\n";
        ...
        fwrite($handle, $delimiter['enclosure'] . implode($delimiter['enclosure'] . $delimiter['delimiter'] . $delimiter['enclosure'], $this->headers) . $delimiter['enclosure'] . $delimiter['line_ending']);
    ...
    public function processTableRow($tablerow, $handle, $sanitize, $delimiter)
    {
        ...
        fwrite($handle, $delimiter['enclosure'] . implode($delimiter['enclosure'] . $delimiter['delimiter'] . $delimiter['enclosure'], $row->toArray()) . $delimiter['enclosure'] . $delimiter['line_ending']);
    }
    ...
    {
        //blade
        if (false !== mb_strpos($column->name, '{{') ||
            false !== mb_strpos($column->name, '{!!')) {
