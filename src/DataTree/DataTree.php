...
$("[data-instance-id=\\"' . $this->attributes['data-instance-id'] . '\\"]").each(function(){
 ...
 root.find(".datatree-inner-wrapper").nestable({
    ...
    protected function performSave()
    {
        ...
        if ($newRoot->getKey() != $root->getKey() || 1 != \count($newTree)) {
        ...
        $left = $rootLeft - 1;
        $depth = $rootDepth;
        $reindex = static function ($tree, $reindex, $depth) use (&$left) {
    ...
    protected function makeRow($item)
    {
        ...
        foreach ($this->columns as $column) {
            ...
            $attrs = [];
            $attrs['data-field-name'] = false === mb_strpos($column->name,
                '{{') ? $column->name : '_blade_' . $index;
